...
class FullyConnected(_LayerBase):
    ...
    @input_dim.setter
    def input_dim(self, previous_layer_output_dim):
        self._input_dim = previous_layer_output_dim
    ...
    def initialize_layer(self, optimizer):
        ...
        assert self.w.shape == (self.output_dim[0], self.input_dim[0])
        assert self.b.shape == (self.output_dim[0], 1)
    ...
    def initialize_parameters(self):
        ...
        self.w, self.b = parameter_initializer(self.output_dim[0], self.input_dim[0])

    def initialize_batch_norm_parameters(self):
        self.gamma = np.ones(shape=(self.output_dim[0], 1))
        self.beta = np.zeros(shape=(self.output_dim[0], 1))
    ...
    def linear_forward(self, input_a):
        ...
        assert input_a.shape[0] == self.input_dim[0], (input_a.shape, self.input_dim)

        z = np.dot(self.w, input_a) + self.b
        assert z.shape == (self.output_dim[0], input_a.shape[1])
        return z
    ...
    def forwards(self, input_a, train):
        ...
        assert self.a.shape == (self.output_dim[0], input_a.shape[1])
        return self.a
    ...
    def backwards(self, da_output=None, mini_batch_size=None, a_l=None, y=None):
        ...
        assert self.dz.shape == self.z.shape
